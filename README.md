...
function parseCurva(rows) {
  // Busca cabeçalho: "Data" ou "PREVISTO" na primeira coluna
  let startRow = 0, colData = 0, colPrev = 1, colReal = 2;
  for (let r = 0; r < Math.min(10, rows.length); r++) {
    const row = rows[r];
    if (!row?.c) continue;
    for (let c = 0; c < row.c.length; c++) {
      const v = String(row.c[c]?.v ?? row.c[c]?.f ?? '').toLowerCase();
      if (v.includes('previsto') || v.includes('acumulado')) {
        startRow = r + 1;
        colData = 0;
        colPrev = c;
        colReal = c + 1;
  ...
  const labels = [], previsto = [], realizado = [];
  for (let r = startRow; r < rows.length; r++) {
    const dateVal = cv(rows, r, colData);
    const prevVal = cv(rows, r, colPrev);
    if (dateVal == null && prevVal == null) continue;
    const lbl = fmtDateShort(dateVal) || String(dateVal ?? '');
    if (!lbl) continue;
    const pv = toNum(prevVal);
    const rv = cv(rows, r, colReal);
    labels.push(lbl);
    previsto.push(pv > 0 ? pv : null);
  ...
  document.getElementById('app').innerHTML = `
    ...
    <div class="card-full">
      ...
      <div class="curva-leg">
        ...
        <div class="curva-leg-item"><div class="curva-leg-line" style="background:#10b981"></div>% Realizado Acumulado</div>
      </div>
